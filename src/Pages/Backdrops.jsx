...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    return (
      <Link to={`/product/${product.id}`} key={product.id}>
        <div className="border border-gray-200 rounded-lg p-4 bg-white shadow-lg">
          {/* Image with Range Badge */}
          <div className="relative w-full h-[500px] sm:h-[450px] lg:h-[400px] mb-4">
            <img
              src={product.image}
              alt={product.name}
          ...
          <p className="text-gray-600 mb-2 text-sm">{product.type}</p>
  ...
  <p className="text-xl font-bold text-purple-600">
    ₦{formatCurrency(product.price)}
          ...
          <button
            className={`flex items-center justify-center px-6 py-3 w-full text-white font-bold rounded-lg transition ${
              product.inStock
